...
print_supported_modes();
  ...
  Eprintf ("-p strg   %d%d%d%d%d%d%d%d\tPensize(s) (in 1/10 mm (mf,ps) or dots (rest)).\n",
  ...
  Eprintf ("-c strg   %d%d%d%d%d%d%d%d\tPen color(s) (see manual for details).\n",
...
NormalWait();
  ...
  Eprintf ("\nPCX-exclusive options:\n");
  Eprintf ("-i         %s\tInvert bitmap\n", FLAGSTATE (po->init_p));
 ...
 **/
  ...
  size_PicBuf (pg, po, &size);

 po->picbuf=allocate_PicBuf(pg, &size);
 if (!po->picbuf) {
